...
function create() {
  ...
  gameState.character = renderCharacter(this, "");
  initializePage(this);
...
function initializePage(scene) {
  // creates options list and background and saves them into gameState

  if (!gameState.options) {
    // creates options list if it doesn't exist
    gameState.options = [];
  }

  if (!gameState.narrative_background) {
    // creates narrative background if it doesn't exist
...
function displayPage(scene, page) {
  ...
  for (let i = 0; i < page.options.length; i++) {
    ...
    // adds in gameplay functionality for options here
...
function fetchPage(page) {
  const pages = [
...

    {
      ...
      page: 5,
      narrative:
        "Orc: My sugar bowl. Goddammit am I hard to understand? I can't reach it by myself but I think you could help.",
      options: [
...

    {
      ...
      page: 19,
      narrative:
        "The orc shares his recipe with you. You leave the cave feeling energized from the sugar. You go on to put the Girl Scouts out of business. \nTHE END",
...

    {
      ...
      options: [{ option: "Yes", nextPage: 12 }, { option: "No", nextPage: 23 }]
    },
